...
import { getAllPermissions, updatePermission } from "../../services/permissionService"
...
const RolePermissions = () => {
  ...
  const checkPermission = (permRecord, moduleName, action) => {
    const moduleEntry = permRecord.modulePermissions?.find((m) => m.moduleName === moduleName)
      ...
      ACTIONS.map((action) => ({
        title: (
          <div className="text-center text-xs">
            <div className="font-bold capitalize">{moduleName}</div>
            <div className="text-gray-400 capitalize">{action}</div>
          </div>
        ),
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <Title level={4} className="m-0!">Role Permissions</Title>
        {!canEdit && (
